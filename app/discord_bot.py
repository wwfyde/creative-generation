...
async def on_message(message: Message):
    ...
    if match:
        ...
        # TODO 将数据发送到队列
        match = re.search(
            rf"{settings.prompt_prefix}(\d+){settings.prompt_suffix}", message.content
        )
        if match:
            ...
            for attachment in message.attachments:
                ...
                if attachment.content_type.startswith("image"):
                    # 将图片上传到阿里云OSS
                    attachment_image_urls = await download_image_to_oss(
                        attachment.url,
                        attachment.filename,
                        is_split=True,
            ...
            async with connection:
                ...
                await exchange.publish(
                    aio_pika.Message(body=body_str.encode()), routing_key=route_key
                )
        ...
        for attachment in message.attachments:
            if attachment.content_type.startswith("image"):
                ...
                await download_image(
                    attachment.url,
                    settings.project_dir.joinpath("assets"),
                    attachment.filename,
                )
    # 监听图像选择
